...
| [01 · The swing](./01-swing.md) §12a | **Cross-engine determinism for `sin`, `cos` and `atan2`.** `Math.hypot` is solved — the simulation writes its own — but the trig is still implementation-approximated, and an orbit clock calls it every tick. Own it, compose rotations instead of re-evaluating at an accumulated angle, or accept single-engine determinism and say so in the recipe. **M1.2 decides and records**; it is the difference between *the simulation is non-deterministic* and *you were running a different engine* |
