...
| [14 · The retro grade](./14-retro-grade.md) §2 vs §3.5 | **A full-screen composite cannot exclude a colour**, and §3.5 plus the acceptance require exactly that of true black — which the anomaly's bed is drawn in today. Every additive stage hits it, not only the lift. ⚠ **Failed by a shipped value since 2026-09-02**: at the ruled grade of 0.45 the gaps sample to `rgb(6, 6, 11)`. Four candidates are tabled in §2's ⚠ notice; **(a)** is [ADR-0011](../adr/0011-canvas2d-carries-the-design.md)'s own open note and is the only one that also buys the gamma and per-channel gain Canvas2D cannot express |
| [14 · The retro grade](./14-retro-grade.md) §3.1 | **Is a track read as a line, or is it structure?** §3.1 gives two floors — 1.5 design px and 1 px — and does not say which a track takes. The deadline's hairline is **1.32**, the only stroke in the game between them; `test/render/strokes.test.ts` asserts the list rather than picking |
| [14 · The retro grade](./14-retro-grade.md) §3.4 vs §2 | **§3.4 forbids texture and §2 stage 5 needs a pattern.** §3.4 is read as binding the world, not the pass; if it binds the pass too, the scanline stage comes out |
...
| 2026-09-03 | **The comb's pitch is 10 design px, not spec 14 §2's 2, and its strength is 0.03.** *"Too weak"* and then *"a bit TOO strong; it distracts from the UX"* on the same 6%, hours apart, with nothing about the ink changed between them — only whether the eye could resolve the bands. At 2 design px the comb is 0.67 CSS px on the author's phone and subtends **1.39 arcminutes**, under the eye's limit, so it integrates to a flat 3% dimming and is not a comb at all; a 240p arcade CRT drew 240 lines where §2's pitch draws 1 266. The **pitch is an overrule** and the **strength is not** — §2 says *"≤ 6%"*, a ceiling rather than a target. The comb also came off the master's gang, taking `SCANLINE_FROM` with it | [14 · §2](./14-retro-grade.md), `src/render/grade.ts` |
| 2026-09-02 | **The retro grade is 0.45**, flown on the bench — *"it looks real nice… 0.45 seems like a nice balance."* Spec 14 §2's five stages are ganged behind one master (§4 asks for exactly that), and **the ruled value sits below the scanline threshold**, so two things follow from it rather than from a nearby number: the pass costs **one** full-screen composite and not two, and §2 stage 5's *"off by default until the phone says otherwise"* is now literally satisfied — the phone has said. Measured on the phone at the *top* of the travel, a frame with one tick costs **1.18 ms against the pre-grade baseline's 1.15**, and the worst frame **7 ms against 10**: the pass at its ceiling does not show above the difference between two runs, which is [ADR-0011](../adr/0011-canvas2d-carries-the-design.md)'s *"the post-processing is free"* reproduced on the shipped renderer. ⚠ It settles the look and **not** §3.5's true black, which is now failed by a shipped value | [14 · §2](./14-retro-grade.md), `src/render/grade.ts`, [M3.5](../plan/m3-the-field.md) |
