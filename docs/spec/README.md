...
| [03 · The HUD](./03-hud.md) §6 | **How a sighting says how far.** Direction 03 rules *"no labels"*; the author asks for distance. Brightness is the only ordinal channel and hue is spent on identity, so distance as brightness is the one answer that needs no label — what is open is whether a sighting steps E0 – E2 with distance or stays flat at E1 |
| [03 · The HUD](./03-hud.md) §6 | **Whether the body a press would take is marked differently.** The prototype rings it at full strength and keeps the ring when the body comes into view; its evidence is a session where the craft was inside the grab window for 1.03s and could see the body for 0.23s of it. A fact about what the button would do rather than an instruction, so nothing refuses it — but it is a second cue in the compass's own job |
