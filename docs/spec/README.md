...
| [14 · The retro grade](./14-retro-grade.md) §2 | **How much grade the game wants.** Built 2026-09-02 as one master with the stages ganged, and it ships at **0**: every number in it is a ceiling the spec states and none is a value anybody has flown. *"A touch more retro"* is a judgement made against the running game, so the opening value is the author's and the knob is on the bench |
| [14 · The retro grade](./14-retro-grade.md) §2 vs §3.5 | **A full-screen composite cannot exclude a colour**, and §3.5 plus the acceptance require exactly that of true black — which the anomaly's bed is drawn in today. Every additive stage hits it, not only the lift. Four candidates are tabled in §2's ⚠ notice; **(a)** is [ADR-0011](../adr/0011-canvas2d-carries-the-design.md)'s own open note and is the only one that also buys the gamma and per-channel gain Canvas2D cannot express |
