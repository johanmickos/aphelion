...
| [04 · Bodies](./04-bodies.md) §2 | **The tide's tracking.** `CONTEXT.md` says it *"always faces the craft"*, §2 says it follows at `k ≈ 6 /s`, and the acceptance asks the lag be bounded and non-zero. Built as written and measured on a settled orbit, the standing lag is **45.3°** against an 18° arc half-width — the tide's near edge sits 27° short of the craft and never overlaps the bearing it is supposed to face. Roughly 15° at `k ≈ 15 /s`. On the bench |
...
| [01 · The swing](./01-swing.md) §12 | **The clock that feeds the tick is quantised, and the craft jumps.** §12 fixes the tick at 1/60s and the catch-up ceiling at 3, and is silent on what resolution the duration arriving at [`ticksDue`](../../src/sim/clock.ts) is measured at. On the author's phone that resolution is **1ms** — WebKit clamps every clock — so the simulation is never told 16.667ms, it is told 16 or 17, and `unspentSeconds` random-walks. Measured over three phone runs: **34 of 1 811 frames ran two ticks and 37 ran none**, arriving in **bursts** (variance ÷ mean of 8, where scatter would give 1), and a burst inside a five-second orbit is what the author reported as *"visual stuttering"*. Reproduced from the real `ticksDue` with no game attached. A five-line fix — treat a duration within the clock's own resolution of a whole number of ticks as that whole number — is measured to take every 60Hz-family display to **zero** spurious jumps while reporting **17 of 17** genuinely dropped frames, and leaves 120Hz untouched. **Flagged, not scheduled**, and it is not a performance change: nothing gets faster, a visible artefact goes away. Evidence and the numbers are in [the performance write-up](../plan/performance.md) §10; determinism is not exposed either way, because `replayRun` never calls `ticksDue` |
