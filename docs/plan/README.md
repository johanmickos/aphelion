...
|---|-----------|-----------|------|--------|
| [M0](./m0-foundations.md) | Foundations, specs, renderer verdict | Specs written, a field you can pan, a renderer ADR | — | **M0.1–M0.4 done** ([specs](../spec/), scaffold, the three layers and `pnpm portable`, the QR dev server) — the QR awaits the author scanning it; CI and the Pages deploy are written but have never run, since the repo has no remote yet and pushing it is deferred; M0.5 not started |
